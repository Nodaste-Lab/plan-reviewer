...
export function renderPlanTemplate(frontmatter: MarkdocFrontmatter, bodyHtml: string, tocEntries: TocEntry[], sourcePath?: string): string {
  ...
  const toc = tocEntries.length
    ? tocEntries.map(entry => `<a href="#${escapeHtml(entry.id)}">${escapeHtml(entry.title)}</a>`).join('')
    : '<span class="muted">No section entries generated.</span>';
  return `<!doctype html>
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .toc { display:grid; grid-template-columns:repeat(auto-fit, minmax(210px, 1fr)); gap:12px; }
...
<main id="${escapeHtml(planId)}" data-generated-from="markdoc" data-markdoc-source="${escapeHtml(sourcePath ?? '')}">
  ...
  <section id="toc"><h2>Contents</h2><div class="toc">${toc}</div></section>
