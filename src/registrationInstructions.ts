...
export function buildRegistrationAgentInstructions(input: { planId: string; reviewUrl: string; serviceUrl?: string }): RegistrationAgentInstructions {
  ...
  return {
    ...
    nextAction: 'Drain pending comments with agent next --no-wait, then listen with agent next --wait before continuing implementation or review work. After a claim, process and ack it before starting another listener. Before implementation, align this plan active/in_progress when building starts. After creating a PR, run plan-review pr link with --service-url and plan-review pr refresh with --url for this plan before final handoff.' ,
    planId: input.planId,
    ...
    processingLoop: [
      ...
      `Before implementation starts for this plan, run plan-review lifecycle set ${input.planId} active --url ${input.serviceUrl ?? '<registration service URL>'}, inspect board columns with plan-review columns list --json --url ${input.serviceUrl ?? '<registration service URL>'}, and set plan-review column set ${input.planId} in_progress --url ${input.serviceUrl ?? '<registration service URL>'} when that column is available and visible. If the in-progress column is missing or ambiguous, stop with an actionable blocker instead of silently building from Backlog or Planning.`,
