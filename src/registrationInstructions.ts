...
export function buildRegistrationAgentInstructions(input: { planId: string; reviewUrl: string; serviceUrl?: string }): RegistrationAgentInstructions {
  ...
  return {
    ...
    processingLoop: [
      ...
      `Before implementation starts for this plan, run plan-review lifecycle set ${input.planId} active --url ${input.serviceUrl ?? '<registration service URL>'}. When board columns are applicable, inspect them with plan-review columns list --json --url ${input.serviceUrl ?? '<registration service URL>'}, then set plan-review column set ${input.planId} in_progress --url ${input.serviceUrl ?? '<registration service URL>'} when that column is available and visible. If Kanban is disabled or the document type does not support board columns, skip only the column alignment and continue after lifecycle is active. If columns are applicable but the in-progress column is missing or ambiguous, stop with an actionable blocker instead of silently building from Backlog or Planning.`,
