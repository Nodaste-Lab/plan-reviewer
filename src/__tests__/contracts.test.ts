...
test('registration instruction helper builds canonical agent-next guidance and rendered commands', () => {
  ...
  assert.match(instructions.nextAction, /Before draining or listening for comments/);
  assert.match(instructions.nextAction, /Then drain pending comments/);
  ...
  assert.match(instructions.nextAction, /in_progress when board columns are applicable/);
  ...
  assert.match(instructions.processingLoop.join('\n'), /Kanban is disabled or the document type does not support board columns, skip only the column alignment/);
  assert.match(instructions.processingLoop.join('\n'), /If columns are applicable but the in-progress column is missing or ambiguous/);
...
test('registration API returns agent instructions additively across registration variants', async () => {
  ...
  try {
    ...
    assert.match(snapshotData.agentInstructions.processingLoop.join('\n'), /skip only the column alignment/);
...
test('Homebrew HEAD deploy guard preserves service-manageable keg metadata', () => {
  ...
  assert.match(script, /verify_keg_metadata repairable/);
