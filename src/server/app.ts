...
function deferredHtml(plans: ReturnType<PlanReviewStore['listPlans']>, archivedCount = 0): string {
  ...
  return `<!doctype html><html><head><meta charset="utf-8"><title>Deferred Plans</title>
    <link rel="icon" type="image/svg+xml" href="/favicon.svg">
    <style>${baseIndexStyles()}${organizationIndexStyles()}</style>
  </head><body><main><div class="topbar"><button class="nav-link" type="button" aria-label="Menu">☰</button>${documentViewSwitcher('all')}<div class="plan-actions">${topbarIconAction('/archive', `Archived (${archivedCount})`, '🗄')}</div></div><div class="page-header"><div><h1>Deferred Plans</h1><p class="muted">Deferred plans are paused for later pickup and keep their notes with the plan.</p></div></div><div class="toolbar"><input id="q" placeholder="Filter deferred plans" aria-label="Filter deferred plans"><select id="repo" aria-label="Filter by repo"><option value="">All repos</option>${repos.map(repo => `<option value="${escapeHtml(repo)}">${escapeHtml(repo)}</option>`).join('')}</select></div><p class="muted" id="deferred-count">${deferredPlans.length} deferred</p><div id="plans">${rows || empty}</div>${rows ? filteredEmpty : ''}<script>
...
function reviewShell(plan: ReturnType<PlanReviewStore['getPlan']>['plan'], currentTitle: string, shellTitle: string, plans: ListedPlan[], columns: BoardColumnRecord[], projectPlans: ListedPlan[]): string {
...
function addArchiveToastDismissListeners(){
  ...
  addArchiveToastDismissTarget(document, 'touchstart', dismiss, true);
  ...
  addArchiveToastDismissTarget(planTouchLayer, 'touchstart', dismiss, true);
  try {
...
function setArchivedShellState(archived){
  ...
  if (status) { status.hidden = !archived; status.textContent = archived ? 'Archived' : ''; }
...
export function createApp(options: AppOptions): FastifyInstance {
  ...
  app.get('/', async (request, reply) => {
    const query = request.query as { q?: string; repoKey?: string; projectKey?: string; status?: string; view?: 'kanban' | 'all' | 'collab' };
    ...
    reply.type('text/html').send(indexHtml(filtered, archivedCount, deferredCount, store.listBoardColumns(), view, projectName));
  });
  ...
  app.get('/p/:planId', async (request, reply) => {
    try {
      ...
      reply.header('Cache-Control', 'no-store').type('text/html').send(reviewShell(plan, title, reviewShellTitle(title), planNavigatorItemsFor(store, { limit: 200, currentPlanId: plan.id }), store.listBoardColumns(), store.listPlans({ includeArchived: true, includeDeferred: true })));
    } catch (error) {
      sendError(reply, error);
    }
  });
