...
function planCardSearch(item: ReturnType<PlanReviewStore['listPlans']>[number]): string {
  const metadata = item.plan.publicationMetadata;
  return `${item.plan.repoName} ${item.plan.repoKey} ${item.plan.slug} ${fullyQualifiedPlanPath(item)} ${metadata.worktreePath} ${metadata.branch} ${metadata.linearIssue ?? ''}`.toLowerCase();
}
