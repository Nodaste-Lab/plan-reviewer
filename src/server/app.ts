...
function reviewShell(plan: ReturnType<PlanReviewStore['getPlan']>['plan'], currentTitle: string, shellTitle: string, plans: ListedPlan[], columns: BoardColumnRecord[], projects: PlanProjectRecord[], configuration: AppConfiguration, navigatorFilters = emptyReviewShellNavigatorFilters()): string {
...
export function createApp(options: AppOptions): FastifyInstance {
  ...
  app.get('/p/:planId', async (request, reply) => {
    try {
      ...
      reply.header('Cache-Control', 'no-store').type('text/html').send(reviewShell(plan, title, reviewShellTitle(title), filteredReviewShellNavigatorItems(store, plan.id, navigatorFilters), columns, projects, configuration, navigatorFilters));
    } catch (error) {
      sendError(reply, error);
    }
  });
