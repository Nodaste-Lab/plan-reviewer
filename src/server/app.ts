...
function kanbanCardHtml(item: ListedPlan, columns: BoardColumnRecord[]): string {
  ...
  return `<article class="kanban-card" draggable="true" tabindex="0" aria-label="Card actions for ${escapeHtml(title)}" data-plan-id="${escapeHtml(item.plan.id)}" data-plan-title="${escapeHtml(title)}" data-column="${escapeHtml(item.plan.boardColumnKey ?? '')}"><div class="plan-card-header"><strong><a href="/p/${escapeHtml(item.plan.id)}">${escapeHtml(title)}</a></strong></div><p class="card-summary">${escapeHtml(summaryForItem(item))}</p>${planBadgesHtml(item, columns)}<a class="card-detail-link" href="/p/${escapeHtml(item.plan.id)}">Details / Open</a></article>`;
}
...
function kanbanIndexHtml(plans: ReturnType<PlanReviewStore['listPlans']>, archivedCount: number, deferredCount: number, columns: BoardColumnRecord[], projectName?: string): string {
  ...
  const board = columns.map(column => `<section class="kanban-column" data-column-key="${escapeHtml(column.key)}" data-column-label="${escapeHtml(column.label)}"><h2>${escapeHtml(column.label)} <span data-column-count>${planning.filter(item => item.plan.boardColumnKey === column.key).length}</span></h2>${cardsFor(column) || '<p class="muted">No plans.</p>'}</section>`).join('\n');
...
function setArchivedShellState(archived){
  ...
  if (status) {
    ...
    if (archived) {
      ...
      status.setAttribute('aria-label', 'Archived');
