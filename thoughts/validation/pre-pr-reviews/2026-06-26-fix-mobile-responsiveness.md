...
- Proxied horizontal touch drags and horizontal wheel/trackpad deltas from the mobile overlay into iframe-local wide scroll regions so table/code/Mermaid/figure overflow is reachable on phones.
...
Late GLM review reported one additional P2 counterexample after PR creation: custom stylesheet-defined grids could be clipped because only known grid class names were collapsed and containers hid overflow. Fixed by adding dynamic iframe wide-region detection that marks arbitrary overflowing rendered elements with `plan-review-wide-scroll`, plus e2e coverage for a non-whitelisted `.feature-grid` touch swipe.
...
- `node dist/test-fixtures/e2e-run.js` — pass.
- `bun run test` — pass.
- `bun run test:e2e` — first rerun hit a known unrelated frame-width equality flake; immediate rerun passed.

Post-fix rereview:

- GPT quality-reviewer — `CLEAN_FOR_PR`.
- GLM quality-reviewer — `CLEAN_FOR_PR`.

PR Codex review then reported two additional P2 issues on the current PR head: horizontal wheel/trackpad input was not proxied into iframe-local wide scroll regions, and plan-authored wide figures without `data-plan-wide` could be scaled down before dynamic wide detection classified them as scrollable. Fixed by adding a horizontal wheel path that only handles dominant horizontal deltas over detected wide regions, preserving native vertical wheel scrolling; by preserving detected wide figure media width inside the local scroller; and by extending the fixture for wide-figure local scroll/touch coverage plus horizontal wheel scrolling.

Scoped GPT rereview found one follow-up P3: the wide figure recovery path appeared to claim generic figure media support while only recovering pre-cap intrinsic width for `<img>`. The renderer only allows plan-authored image figures through sanitization, so the implementation and notes were narrowed to supported image figures instead of carrying dead canvas/video/SVG support.

A follow-up Codex review on PR head `6f5dda6` found one additional P2: mobile wide-image sizing was stored inline and not cleared after resizing back to desktop. Fixed by storing/restoring the original image `min-width`, clearing dynamic wide-scroll state outside mobile mode, and adding resize cleanup coverage.

Final verification after narrowing and stabilizing the short-height Kanban fixture:
