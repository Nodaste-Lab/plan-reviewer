...
- PR feedback identified that automatic delivery inserted comment lifecycle events into storage but did not emit them on the in-process event bus for already-connected SSE clients. The worker now accepts the app event bus and emits the `StoredEvent`s returned by claim, ack, resolve, release, and expired-claim store operations.
